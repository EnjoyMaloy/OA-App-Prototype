      ...
      <div className="relative z-10 max-w-full mx-auto px-3 md:px-9 flex items-center justify-between h-full gap-2 md:gap-4 pt-2 md:pt-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="md:hidden flex items-center gap-2.5 min-w-0">
            {avatarBlock}
            <span className="text-[17px] font-medium text-foreground truncate">{t("home.greeting")}</span>
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {!hasSubscription && (
            <>
              {/* Desktop: full button */}
              <div className="hidden md:block rounded-[10px] p-[2px] animate-gradient-border h-12" style={{ background: "linear-gradient(270deg, #924CFE, #BF96FF, #D9C0FF, #A66CFF, #924CFE)", backgroundSize: "300% 300%" }}>
                <button
                  onClick={() => setSubModalOpen(true)}
                  className="flex items-center gap-2 rounded-[8px] bg-[#A66CFF] text-white text-[16px] font-normal hover:brightness-110 transition-all w-full h-full px-4"
                >
